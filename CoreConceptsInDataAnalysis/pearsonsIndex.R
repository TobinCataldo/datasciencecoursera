## pearsonsIndex.R
## generates Pearson's Index from a contingency table, actually
...
pearsonsIndex <- function (myCumRelFreqTable, myExpFreqTable) {
    ##
    ## Difference between this and pearsonsChiSquared
    ## This                           pearsonsChiSquared  
    ...
    for (h in 1:(tColsCumRel)) {
        ...
        for (i in 1:(tRowsCumRel)) {
            ...
            mat <- rbind(mat, (myCumRelFreqTable[i,h] - myExpFreqTable[i,h]) / sqrt(myExpFreqTable[i,h]))
        } 
        if (is.na(mat[1,1])) {
